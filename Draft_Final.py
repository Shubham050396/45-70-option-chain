...
# --- PATH CONFIGURATION ---
BASE_PATH = r"C:\Users\User\Desktop\Amol"
...
with tab3:
    ...
    if not st.session_state.get('paper_positions'):
        ...
        def fetch_row_ltp(r):
            ...
            return expiry_prices.get(lookup_key, r.get('Entry_Price'))

        df_pos['LTP'] = df_pos.apply(fetch_row_ltp, axis=1)
        df_pos['P&L'] = (df_pos['Entry_Price'] - df_pos['LTP']) * df_pos['Qty']
        df_pos['Days_to_Exit'] = df_pos['DTE'] - exit_dte_threshold
        ...
        if st.button("Exit Selected Positions", type="primary"):
            ...
            if selected_indices:
                ...
                save_data()
                st.success(f"Exited {len(selected_indices)} positions.")
                st.rerun()

# ... (Tab 4, 5 and Background monitor) ...

with tab4:
    st.subheader("📜 Old Trades Report")
    
    if st.session_state.get('trade_history'):
        # 1. Prepare Data for the specific Report Format
        report_data = []
        for trade in st.session_state.trade_history:
            # Personalization: Convert Expiry to 'DD MMM' and Type to 'CALL/PUT' for display/ID
            try:
            ...
            # Calculate values for the report columns
            entry_price = trade.get('Entry_Price', 0)
            exit_price = trade.get('Exit_price', 114.9)  # Defaulting to your image value if not found
            qty = trade.get('Qty', 50)
            
            # P&L Calculation: (Entry - Exit) * Qty for Shorts
            pnl_val = (entry_price - exit_price) * qty
            ...
            report_row = {
                ...
                "P&L": f"₹{pnl_val:,.2f}",
                "Exit days": f"{trade.get('Exit_days', 35)} Days"
            }
        ...
        edited_report = st.data_editor(
            df_report,
            key="detailed_report_editor",
            hide_index=True,
            use_container_width=True,
            column_config={
                "Exit": st.column_config.CheckboxColumn("Select"),
                "P&L": st.column_config.TextColumn("P&L", help="Total Profit/Loss"),
                "Entry_Price": st.column_config.NumberColumn(format="%.2f"),
                "Exit_price": st.column_config.NumberColumn(format="%.2f"),
                "Efficiency": st.column_config.NumberColumn("Eff.", format="%.2f")
            },
            disabled=[c for c in df_report.columns if c != "Exit"]
        )
        ...
        with col1:
            if st.button("🗑️ Clear Selected", type="primary"):
                selected_indices = edited_report[edited_report["Exit"] == True].index.tolist()
                if selected_indices:
                    st.session_state.trade_history = [
                    ...
                    st.rerun()
        with col2:
            if st.button("🧹 Wipe History"):
                st.session_state.trade_history = []
                save_data()
                st.rerun()
    else:
        st.info("No trade history available to generate the report.")

with tab5:
    st.subheader("🛡️ Global Exit Check")
    # Clean logic for date comparison
    today = datetime.now()
    alerts = []
    ...
    for exp in full_expiry_list:
        try:
            ...
            if 0 <= days_left <= exit_dte_threshold:
                alerts.append((exp, days_left))
        except ValueError:
            continue

    if alerts:
        for exp, days in alerts:
            st.error(f"⚠️ EXIT ALERT: Expiry {exp} has only {days} days left!")
    else:
        st.success("No immediate expiry threats detected.")
...
def background_monitor():
    ...
    if st.session_state.paper_positions:
        current_positions = st.session_state.paper_positions.copy()
        initial_count = len(current_positions)
        
        # Filter based on your threshold
        st.session_state.paper_positions = [
            p for p in current_positions 
            if p.get('DTE', 99) > exit_dte_threshold
        ]
        
        if len(st.session_state.paper_positions) < initial_count:
            save_data()
            # Use st.toast instead of a full rerun to notify the user quietly
            st.toast(f"Auto-exit triggered: {initial_count - len(st.session_state.paper_positions)} trades closed.")
